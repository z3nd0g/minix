...
#define SCHED_RATE (MILLISEC*HZ/1000)	/* number of ticks per schedule, kwant dla grupy C; HZ=60, więc SCHED_RATE = 6msec */
#define GROUP_A_RATE  (5*SCHED_RATE)  /* kwant dla grupy A*/
#define GROUP_B_RATE  (10*SCHED_RATE)  /* kwant dla grupy B*/
 ...
 */
  ...
  if (--sched_ticks == 0) {	
	if (bill_ptr == prev_ptr) lock_sched();	/* process has run too long; lock_sched() = Safe gateway to sched() for tasks.; robi sched() */
	if (rp->p_group == 'A')
	    sched_ticks = GROUP_A_RATE;
	else if (rp->p_group == 'B')
	    sched_ticks = GROUP_B_RATE;
	else    	
 ...
 */
  ...
  if (k_reenter != 0)
	...
	rp = proc_ptr; /*kernel/glo.h:EXTERN struct proc *proc_ptr = pointer to currently running process */
  ...
  if (--sched_ticks == 0) {
	...
	if (rp->p_group == 'A')
	    sched_ticks = GROUP_A_SCHED_RATE;
	else if (rp->p_group == 'B')
	    sched_ticks = GROUP_B_SCHED_RATE;
	else
	    sched_ticks = SCHED_RATE;	/* Grupa C - reset quantum */			
