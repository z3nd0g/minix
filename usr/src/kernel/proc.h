...
struct proc {
  ...
  char p_group;			/* grupa procesu (A,B,C) */
...
#define BEG_PROC_ADDR (&proc[0]) /*adres poczatku tablicy procesow*/
...
#define proc_vir2phys(p, vir) \
			  (((phys_bytes)(p)->p_map[D].mem_phys << CLICK_SHIFT) \
							+ (vir_bytes) (vir))
#define DEFAULT_GROUP 'C'	

EXTERN struct proc proc[NR_TASKS + NR_PROCS];	/* PROCESS TABLE; z include/minix.const.h: NR_TASKS = (8 + NR_CTRLRS + ENABLE_DP8390 + ENABLE_PRINTER + 2 * ENABLE_SB16) , z include/minix/config.h NR_PROCS = 31 - Number of slots in the process table for user processes*/
...
EXTERN struct proc *rdy_head[NQ];	/* pointers to ready list headers; z const.h NQ = 3 - #of scheduling groups */
