 ...
 */
  ...
  if (rdy_head[USER_Q] == NIL_PROC) return; /*  NIL_PROC = ((struct proc *) 0), USER_Q = 2 */

  /* One or more user processes queued. */
  
  curr_gr = rdy_head[USER_Q]->p_schedgroup;
  /* znajdujemy indeks tablicy groups, odpowiadający grupie bieżącego procesu */
  while (groups[i]!=curr_gr
    i++;
  /* określamy następną grupę */
  next_gr = groups[(++i)%3];

  /*switch(curr_gr){
  case 'A': next_gr == 'B'; break;
  case 'B': next_gr == 'C'; break;
  case 'C': nest_gr == 'A'; break;
  }*/

  /*przenosimy bieżący(obsłużony już) proces na koniec kolejki */
  ...
  /*szukamy procesu z następnej grupy i wstawiamy go na początek kolejki*/
  /* Najpierw sprawdzamy czy głowa listy nie zawiera takiego procesu. Jeśli tak, to już nie musimy dalej szukać. */
  if (rdy_head[USER_Q]->p_group != next_gr && rdy_head[USER_Q]->p_nextready != NIL_PROC) {
    
      while (p->p_group != next_gr && j<2) {
	    tmp = rdy_head[USER_Q];
	    p = rdy_head[USER_Q]->p_nextready;
	    
	    while (p->p_group != next_gr && p_nextready != NIL_PROC) {
	      tmp = tpm->p_nextready;
	      p = p->p_nextready;
	    }
	 
	    /* Znaleźliśmy proces z następnej grupy. Przenosimy go na początek kolejki. */
	    if (p->p_group == next_gr) {
	      tmp->p_nextready = p->p_nextready;
	      p->p_nextready = rdy_head[USER_Q];
	      rdy_head[USER_Q] = p;
	      free(tmp);
	    }
	    /* nie znależliśmy takiego procesu, szukamy procesu z kolenej grupy */
	    next_gr = groups[(++i)%3];
	    j++;
       }
    }
